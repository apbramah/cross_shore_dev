...
camera_power_en.value(1) # Force power enable through to lens
...
async def _send_lens_state():
    if not ws:
        return
    await ws.send(json.dumps({
        "type": "CURRENT_LENS_TYPE",
        "uid": uid_hex,
        "lens_type": lens.get_lens_type(),
    }))
    await ws.send(json.dumps({
        "type": "CURRENT_LENS_AXIS_SOURCES",
        "uid": uid_hex,
        "sources": lens.get_axis_sources(),
    }))
...
async def _camera_com_tx_task():
    """Read raw bytes from lens UART and send to controller via COM_DATA websocket messages."""
    global ws, com_peer_uid
    while True:
        try:
            data = lens.read_raw()
...
async def _lens_periodic_task():
    while True:
        try:
            ...
            print("Error in _lens_periodic_task:", e)
            await asyncio.sleep(0.1)
...
async def on_reliable_message(data):
    ...
    try:
        ...
        if msg_type == "SET_LENS_TYPE":
            lens_type = str(msg.get("lens_type", "")).lower()
            if lens.set_lens_type(lens_type):
                await _send_lens_state()
                print(f"SET_LENS_TYPE handled over reliable channel -> lens_type={lens_type}")
            else:
                print(f"Unknown lens type requested: {lens_type}")
            return

        if msg_type == "SET_LENS_AXIS_SOURCE":
            axis = str(msg.get("axis", "")).lower()
            source = str(msg.get("source", "")).lower()
            if lens.set_axis_source(axis, source):
                await _send_lens_state()
                print(f"SET_LENS_AXIS_SOURCE handled -> axis={axis} source={source}")
            else:
                print(f"Invalid axis/source requested: axis={axis} source={source}")
            return

        print(f"Reliable channel received (json): {msg}")
    except Exception as e:
        print(f"Error handling reliable channel message: {e}")
...
async def websocket_client(ws_connection, server_url=None):
    ...
    try:
        ...
        lens_periodic_task = asyncio.create_task(_lens_periodic_task())
        ...
        while True:
            ...
            elif my_dict["type"] == "SET_LENS_TYPE":
                lens_type = str(my_dict.get("lens_type", "")).lower()
                if lens.set_lens_type(lens_type):
                    await _send_lens_state()
                else:
                    print(f"Unknown lens type requested over websocket: {lens_type}")
            elif my_dict["type"] == "SET_LENS_AXIS_SOURCE":
                axis = str(my_dict.get("axis", "")).lower()
                source = str(my_dict.get("source", "")).lower()
                if lens.set_axis_source(axis, source):
                    await _send_lens_state()
                else:
                    print(f"Invalid lens source request over websocket: axis={axis} source={source}")
        ...
        try:
            ...
            pass
        try:
            if lens_periodic_task:
                lens_periodic_task.cancel()
                await lens_periodic_task
        except Exception:
            pass
        ws = None
