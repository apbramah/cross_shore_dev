...
- **Position Display opens but map is blank/no tiles**  
  First confirm the standalone file exists in runtime:
  ```bash
  ls -l /opt/ui/position_map_standalone.html
  sha256sum ~/Dev/cross_shore_dev/apps/controller/position_map_standalone.html /opt/ui/position_map_standalone.html
  ```
  If hashes match and tiles are still missing, this is likely outbound CDN/network blocking on the Pi. Current standalone map includes tile-provider failover and an on-screen tile status message to make this visible.
...
2. **Pi:** Pull in `~/Dev/cross_shore_dev`; deploy with `sudo install -m 644` (including `position_map_standalone.html`, no `rsync --delete`); ensure `boot.html` exists; restart `wsbridge` (and kiosk if needed).  
...
## Working now (2026-03-15)

- Head IP programming from Head ID Configuration is working with transaction-mode slow-channel handling and validation.
- Position map is working when runtime file hashes match source and `/opt/ui/position_map_standalone.html` is present.

### Controller runtime verification snippet
...
checks = {
  ...
  "/opt/wsbridge/mvp_slow_bridge.py": ["_send_network_config_mode_packet", "head_ack_inferred"],
  "/opt/ui/position_map_standalone.html": ["POSITION_DISPLAY_UPDATE", "setMapStyle"],
}
